...
#
# Bu betik yalnızca Intel Mac (x86_64) içindir. Apple Silicon (M1/M2/M3/M4)
# bir Mac'teyseniz bunun yerine run-mac-apple-silicon.sh kullanın:
#   ./run-mac-apple-silicon.sh
#
# Kullanım:
#   ./run-mac-intel.sh        → n8n'i başlatır (http://localhost:5678)
#   ./run-mac-intel.sh --ai   → n8n + yerel AI (Ollama) profilini başlatır
#   ./run-mac-intel.sh --stop → n8n'i durdurur (veriler korunur)
#
...
if ! command -v docker >/dev/null 2>&1; then
  ...
  echo "  ve \"Mac with Intel chip\" (Intel Chip) düğmesini indirin."
  echo ""
  echo "  Apple Silicon (M1/M2/M3/M4) bir Mac kullanıyorsanız bu betik değil,"
  echo "  run-mac-apple-silicon.sh betiğini kullanın."
  echo ""
  echo "  Kurduktan sonra Docker Desktop uygulamasını bir kez açın, sonra bu"
  echo "  betiği tekrar çalıştırın: ./run-mac-intel.sh"
  exit 1
fi
...
echo ""
echo "  Durdurmak için:  ./run-mac-intel.sh --stop   (veya: docker compose down)"
